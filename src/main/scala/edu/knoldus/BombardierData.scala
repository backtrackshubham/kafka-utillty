  ...
  def publishImageHeader = Future {
    cameraIds.foreach(camera => {
      ...
      imagesPerCamera.zipWithIndex.foreach { case (file, index: Int) =>
        ...
        DataProducer.writeToKafka("image_header_aug_2018", camera, write(imageHeaderData.copy(timestamp = System.currentTimeMillis(), imageId = s"$imageId-$index", cameraId = camera)))
        DataProducer.writeToKafka("image_header_aug_2018", s"$imageId-$index-L.png", byteArray)
        DataProducer.writeToKafka("image_header_aug_2018", s"$imageId-$index-R.png", byteArray)
        Thread.sleep(100)
      }
    })
  }
  ...
  def publishGPSData = Future {
    cameraIds foreach {camera =>
      imagesPerCamera.foreach { _ =>
        ...
        DataProducer.writeToKafka("camera_gps_aug_2018", camera, write(gpsData.copy(timestampLinux = System.currentTimeMillis(), cameraId = camera)))
        Thread.sleep(100)
      }
    }
  }
  ...
  def publishIMUData = Future {
    cameraIds foreach {camera =>
      imagesPerCamera.foreach { _ =>
        ...
        (1 to 10) foreach { _ =>
          DataProducer.writeToKafka("camera_imu_aug_2018", camera, write(imuData.copy(timestampLinux = System.currentTimeMillis(), cameraId = camera)))
          Thread.sleep(10)
        }
      }
    }
